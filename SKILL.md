...
### 支持的格式
- `.epub` → 用 `read_book.py` 直接读取章节原文
- `.pdf`  → 用 `read_book.py` 提取文本（需先 `pip install pypdf`）
...
python3 read_book.py "D:\电子书\无穷的开始：世界进步的本源（第2版）.epub" "更接近现实" --limit 15000
```
...
```json
{
  ...
  "chars": 15234,
  "text": "第2章 更接近现实 星系是大得令人难以置信的物体..."
}
```

### 工作流程
1. 用户告知本地电子书路径
2. 调用 `read_book.py` 读取指定章节（关键词 = 章节标题）
3. 基于**原文内容**撰写学习笔记（不再靠搜索推断）
4. 引用时标注证据：`证据：A=电子书原文(partXXXX.html)`

### 注意事项
- EPUB 是 ZIP 格式，`read_book.py` 通过 zipfile 模块直接解析，不需要解压
- 若关键词匹配到多个文件，脚本返回第一个匹配项（通常是章节正文）
...
**触发条件**：用户提供了电子书本地路径，或说「我在 XX 下载到了这本书」

**执行步骤**：
1. 用 `read_book.py` 读取目录页（关键词=`目录` 或 `Contents`）
2. 获得全书章节列表
3. 直接进入阶段一（基于原文，不再靠搜索）
